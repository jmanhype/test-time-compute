...
class InferenceWithGists:
    ...
    def _should_stop(self, 
                    ...
                    token_id: int,
                    start_time: float,
                    config: GenerationConfig) -> Tuple[bool, Optional[StopReason]]:
        """
    ...
    def generate_with_gists(self,
                           prompt: Union[str, List[str]],
                           config: Optional[GenerationConfig] = None) -> Union[
        ...
        try:
            ...
            for step in range(config.max_length):
                ...
                token_prob = self._get_token_probability(next_token_logits, next_token.item())
    ...
    def analyze_gists(self, gists: List[Gist]) -> Dict[str, float]:
        ...
        analysis = {
            ...
            "tokens_per_second": len(gists) / generation_time if generation_time > 0 else 0
        }
        
        return analysis
    
    def get_generation_trace(self, gists: List[Gist], interval_seconds: float = 0.1) -> List[Dict]:
        """
        ...
        while current_time <= gists[-1].timestamp:
            ...
            while (last_gist_idx < len(gists) and 
                   gists[last_gist_idx].timestamp <= current_time):
                last_gist_idx += 1
